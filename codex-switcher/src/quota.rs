    ...
    if let Ok(resp) = req.call() {
        if let Ok(json_val) = resp.into_json::<serde_json::Value>() {
            if let Some(accounts_map) = json_val.get("accounts").and_then(|a| a.as_object()) {
                let target_acc = accounts_map.values().find(|item| {
                    if let Some(sub_id) = item.get("account").and_then(|a| a.get("id")).and_then(|id| id.as_str()) {
                ...
                if let Some(item) = target_acc {
                    let sub_plan = item.get("entitlement")
                        .and_then(|e| e.get("subscription_plan"))
                        .and_then(|p| p.as_str())
                        .unwrap_or("")
                        .to_lowercase();

                    let plan_type = item.get("account")
                        .and_then(|a| a.get("plan_type"))
                        .and_then(|p| p.as_str())
                        .unwrap_or(&account.plan_type)
                        .to_lowercase();

                    let features = item.get("features")
                        .and_then(|f| f.as_array())
                        .map(|arr| arr.iter().filter_map(|v| v.as_str()).collect::<Vec<_>>())
                        .unwrap_or_default();

                    if plan_type == "pro" {
                        if sub_plan.contains("100") || sub_plan.contains("lite") || sub_plan.contains("5x") {
                            detected = 5.0;
                        } else {
                            detected = 20.0;
                        }
                    } else if plan_type == "team" || plan_type == "business" {
                        if features.iter().any(|&f| f == "self_serve_business_prolite" || f.contains("premium_seat")) {
                            detected = 5.0;
                        } else {
                            detected = 1.0;
                        }
...
pub fn update_account_quota_cache(account: &mut AccountConfig) {
    ...
    let should_detect = account.multiplier_is_manual != Some(true) && (
        ...
        account.last_multiplier_checked.as_deref().map_or(true, |ts| {
        ...
        Ok(usage) => {
            ...
            if let Some(rl) = usage.rate_limit {
                if let Some(prim) = rl.primary_window {
                    if account.multiplier_is_manual != Some(true) && (account.plan_type == "team" || account.plan_type == "business") {
