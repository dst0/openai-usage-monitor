...
5. **Desktop Application Switching (`ChatGPT.app`)**:
   ...
   - `cxi window probe-tasks --allow-focus-and-clipboard` is an explicit diagnostic for the missing window-to-task mapping; it proves nothing about restart or restoration. Before any visible change it requires the flag, the switch/recovery operation lock, which it holds until it finishes (a switch or recovery already running refuses the probe, and one started during the probe is refused), a default ChatGPT keymap (`$CODEX_HOME/keybindings.json` absent, blank, or `[]`), exactly one ChatGPT process, Accessibility and event-posting access, and no minimized ChatGPT window. It then focuses each ChatGPT window, sends Cmd+Opt+L, the default Copy deeplink shortcut in ChatGPT 26.924.20706 (other builds are unverified), only to that ChatGPT process, and leaves the last copied link on the clipboard; clipboard history tools may keep every copied link. Each copy requires observed keyboard focus on that window, an unchanged process birth, unambiguous AX/WindowServer geometry, a stable window inventory and mapping, exactly one clipboard write, and a unique canonical task link. The CLI prints only a window count or a fixed failure code; task IDs never leave the helper. A single competing clipboard write of a valid task link remains indistinguishable, so attribution is unverified. The probe saves no restart snapshot and does not relax the multiwindow guard. `Task probe failed: COMMAND_REJECTED` means the installed window helper predates the probe; rerun the installer.
