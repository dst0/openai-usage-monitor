...
5. **Desktop Application Switching (`ChatGPT.app`)**:
   ...
   - `cxi window probe-tasks --allow-focus-and-clipboard` explicitly probes the missing window-to-task mapping. It focuses each ChatGPT window, invokes Copy deeplink, and leaves the last copied link on the clipboard. It checks process birth, unambiguous AX/WindowServer geometry, stable window inventory and mapping, one observed clipboard write per copy, and unique canonical task IDs. The helper returns only process identity, window IDs and a count; the CLI prints only that count. A single competing clipboard write containing a valid task URL remains indistinguishable from the shortcut. The probe saves no restart snapshot and does not relax the multiwindow guard.
