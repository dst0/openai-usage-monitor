...
public struct MenuBarAppearanceHelper {
  ...
  public static func isCurrentScreenActive(itemWindow: NSWindow?) -> Bool {
    ...
    let itemID =
      (itemScreen.deviceDescription[NSDeviceDescriptionKey("NSScreenNumber")] as? NSNumber)?
      .uint32Value
    let activeID =
  ...
  public static func makeGlossySplitCircle(
    ...
    return NSImage(size: NSSize(width: diameter, height: diameter), flipped: false) { rect in
      ...
      let leftCGColors =
        leftType.colors(isScreenActive: isScreenActive).map { $0.cgColor } as CFArray
      if let leftGrad = CGGradient(
        colorsSpace: colorSpace, colors: leftCGColors, locations: [0.0, 0.55, 1.0])
      {
        ctx.drawLinearGradient(
          leftGrad, start: CGPoint(x: 0, y: diameter), end: CGPoint(x: 0, y: 0), options: [])
      }
      ctx.restoreGState()
      ...
      let rightCGColors =
        rightType.colors(isScreenActive: isScreenActive).map { $0.cgColor } as CFArray
      if let rightGrad = CGGradient(
        colorsSpace: colorSpace, colors: rightCGColors, locations: [0.0, 0.55, 1.0])
      {
        ctx.drawLinearGradient(
          rightGrad, start: CGPoint(x: 0, y: diameter), end: CGPoint(x: 0, y: 0), options: [])
      }
      ctx.restoreGState()
      ...
      let glossRect = CGRect(
        x: diameter * 0.18, y: diameter * 0.52, width: diameter * 0.64, height: diameter * 0.40)
      ...
      let glossColors =
        [
          NSColor(white: 1.0, alpha: highlightAlpha).cgColor,
          NSColor(white: 1.0, alpha: 0.0).cgColor,
        ] as CFArray
      if let glossGrad = CGGradient(
        colorsSpace: colorSpace, colors: glossColors, locations: [0.0, 1.0])
      {
        ctx.drawLinearGradient(
          glossGrad, start: CGPoint(x: 0, y: diameter * 0.92),
          end: CGPoint(x: 0, y: diameter * 0.52), options: [])
      }
      ctx.restoreGState()
  ...
  public static func makeHybridQuotaIndicator(
    ...
    return NSImage(size: NSSize(width: width, height: height), flipped: false) { rect in
      ...
      let contour = CGPath(
        roundedRect: badgeRect, cornerWidth: cornerR, cornerHeight: cornerR, transform: nil)
      ...
      let topPath = CGPath(
        roundedRect: topRect, cornerWidth: cellR, cornerHeight: cellR, transform: nil)
      ctx.saveGState()
      ctx.addPath(topPath)
      ctx.clip()
      let f5hColors = f5hType.colors(isScreenActive: isScreenActive).map { $0.cgColor } as CFArray
      if let grad = CGGradient(
        colorsSpace: colorSpace, colors: f5hColors, locations: [0.0, 0.55, 1.0])
      {
        ctx.drawLinearGradient(
          grad, start: CGPoint(x: defaultX, y: topRect.maxY),
          end: CGPoint(x: defaultX, y: topRect.minY), options: [])
      }
      ctx.restoreGState()
      ...
      let midPath = CGPath(
        roundedRect: midRect, cornerWidth: cellR, cornerHeight: cellR, transform: nil)
      ctx.saveGState()
      ctx.addPath(midPath)
      ctx.clip()
      let wColors = wType.colors(isScreenActive: isScreenActive).map { $0.cgColor } as CFArray
      if let grad = CGGradient(
        colorsSpace: colorSpace, colors: wColors, locations: [0.0, 0.55, 1.0])
      {
        ctx.drawLinearGradient(
          grad, start: CGPoint(x: defaultX, y: midRect.maxY),
          end: CGPoint(x: defaultX, y: midRect.minY), options: [])
      }
      ctx.restoreGState()
      ...
      let botPath = CGPath(
        roundedRect: botRect, cornerWidth: cellR, cornerHeight: cellR, transform: nil)
      ctx.saveGState()
      ctx.addPath(botPath)
      ctx.clip()
      let cColors = cType.colors(isScreenActive: isScreenActive).map { $0.cgColor } as CFArray
      if let grad = CGGradient(
        colorsSpace: colorSpace, colors: cColors, locations: [0.0, 0.55, 1.0])
      {
        ctx.drawLinearGradient(
          grad, start: CGPoint(x: defaultX, y: botRect.maxY),
          end: CGPoint(x: defaultX, y: botRect.minY), options: [])
      }
      ctx.restoreGState()
  ...
  public static func makeBoostedIcon(from baseImage: NSImage, isScreenActive: Bool = true)
    -> NSImage
  {
  ...
  public static func makeColoredProgressBar(
    label: String, percentage: Double, maxPercentage: Double = 100.0, fillColor: NSColor
    ...
    if empty > 0 {
      result.append(
        NSAttributedString(
          string: String(repeating: "░", count: empty),
          attributes: [
            .font: barFont,
  ...
  public static func makeSprintIcon(size: CGFloat = 8.0, isScreenActive: Bool = true) -> NSImage {
    ...
    let img = NSImage(size: NSSize(width: totalW, height: totalH), flipped: false) { rect in
      ...
      let colorSpace = CGColorSpaceCreateDeviceRGB()
      if let grad = CGGradient(
        colorsSpace: colorSpace, colors: colors, locations: [0.0, 0.50, 1.0])
      {
  ...
  public static func makeStackedValuesImage(
    ...
    let sprintIconSample =
      useQuotaIcons ? makeSprintIcon(size: 8.0, isScreenActive: isScreenActive) : nil
    let sprintIconW: CGFloat = sprintIconSample?.size.width ?? 10.0
    let sprintIconH: CGFloat = sprintIconSample?.size.height ?? 8.0
    let weeklyIconSample =
      useQuotaIcons ? makeWeeklyIcon(size: 10.0, isScreenActive: isScreenActive) : nil
    ...
    let img = NSImage(size: NSSize(width: totalWidth, height: totalHeight), flipped: false) {
      rect in
      ...
      if useQuotaIcons {
        let sprintIcon =
          sprintIconSample ?? makeSprintIcon(size: 8.0, isScreenActive: isScreenActive)
      ...
      if useQuotaIcons {
        let weeklyIcon =
          weeklyIconSample ?? makeWeeklyIcon(size: 10.0, isScreenActive: isScreenActive)
