...
- **Verification:** Pure Swift link validation tests and Rust helper-response tests run without live Desktop state; native helper compilation verifies its API. No live UI probe or restoration proof is claimed.
...
- **References:** `scripts/CodexWindowTaskProbe.swift`, `codex-switcher/src/distribution/system_window_restore_backend.rs`, `codex-switcher/src/switcher/codex_app_lifecycle.rs`.
