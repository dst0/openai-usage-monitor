...
- **Resolution:** Moved the banner tests into the crate. The same pass fixed the other pre-existing `--all-targets` failures: a `TestAccountSpec` parameter struct replaced the 9-argument `make_account`, `Settings` literals use struct-init syntax, The `DistributionDesktopRelaunchService::run` arity error was resolved upstream by #12, which moved session binding out of `run`; the `codex_binary_path.rs` `from_ref` lint and an in-crate move of the banner test also landed upstream in parallel (#15), and this branch replaced #15's `#[expect(clippy::too_many_arguments)]` on `make_account` with `TestAccountSpec`.
