...
- **Resolution:** Recovery in an already running Desktop now uses WindowServer banner placement without geometry restoration. Missing-window and panel visibility timeout failures permit IPC recovery; identity is checked again after an optional capture failure. Changed identity, missing helper, payload/lease errors, and invalid helper output remain blocking.
- **Verification:** The focused backend regression invokes only `inspect-process` and `capture-banner-window`, continues after `WINDOW_NOT_FOUND`, and rejects malformed output and a changed process birth ID after `WINDOW_ACCESS_FAILED`. Full installed cold quota recovery remains pending.
