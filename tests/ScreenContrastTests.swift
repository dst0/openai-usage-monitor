...
struct ScreenContrastTestRunner {
  static func main() {
    ...
    assertTrue(
      inactiveGreenTop.greenComponent >= 0.80, "Inactive green top highlight green tone >= 0.80")
    assertTrue(
      inactiveGreenTop.greenComponent <= 0.90,
      "Inactive green top must not be neon oversaturated <= 0.90")

    // Inactive red top highlight: warm red tint
    let inactiveRedTop = redInactiveColors[0].usingColorSpace(.sRGB)!
    assertTrue(
      inactiveRedTop.redComponent >= 0.85, "Inactive red top highlight must have strong red >= 0.85"
    )
    ...
    assertTrue(
      abs(activeGrayMid.redComponent - activeGrayMid.greenComponent) < 0.05,
      "Active gray should be neutral")
    assertTrue(
      abs(activeGrayMid.redComponent - activeGrayMid.blueComponent) < 0.05,
      "Active gray should be neutral")
    assertTrue(
      abs(inactiveGrayMid.redComponent - inactiveGrayMid.greenComponent) < 0.05,
      "Inactive gray mid should be neutral")
    assertTrue(
      abs(inactiveGrayMid.redComponent - inactiveGrayMid.blueComponent) < 0.05,
      "Inactive gray mid should be neutral")
    assertTrue(
      inactiveGrayMid.redComponent <= activeGrayMid.redComponent,
      "Inactive gray mid must not be brighter than active")
    ...
    assertTrue(
      MenuBarAppearanceHelper.isWeeklyExhausted(0.245), "0.245% micro-remnant is exhausted")
    ...
    assertTrue(
      !MenuBarAppearanceHelper.isWeeklyExhausted(Double.nan), "NaN weekly is not exhausted")

    // Exhausted weekly quota overrides 100% 5h to slate gray
    let grayActive = MenuBarAppearanceHelper.menuBarColor(
      forPercentage: 100.0, weeklyPercentage: 0.0, isScreenActive: true
    ).usingColorSpace(.sRGB)!
    let grayInactive = MenuBarAppearanceHelper.menuBarColor(
      forPercentage: 100.0, weeklyPercentage: 0.0, isScreenActive: false
    ).usingColorSpace(.sRGB)!
    let grayRemnant = MenuBarAppearanceHelper.menuBarColor(
      forPercentage: 100.0, weeklyPercentage: 0.245, isScreenActive: true
    ).usingColorSpace(.sRGB)!
    let greenActive = MenuBarAppearanceHelper.menuBarColor(
      forPercentage: 100.0, weeklyPercentage: 50.0, isScreenActive: true
    ).usingColorSpace(.sRGB)!

    assertTrue(
      abs(grayActive.redComponent - grayActive.greenComponent) < 0.05,
      "Exhausted weekly color must be slate gray")
    assertTrue(
      greenActive.greenComponent > greenActive.redComponent + 0.3,
      "Healthy weekly color must be bright green")
    assertEqual(
      grayActive.redComponent, grayRemnant.redComponent,
      "Remnant weekly should yield identical gray to 0.0%")
    assertTrue(
      grayInactive.redComponent <= grayActive.redComponent,
      "Inactive gray must not be brighter than active gray")

    // Inactive red color check: low-quota percentage on inactive screen
    let activeRed = MenuBarAppearanceHelper.menuBarColor(forPercentage: 19.0, isScreenActive: true)
      .usingColorSpace(.sRGB)!
    let inactiveRed = MenuBarAppearanceHelper.menuBarColor(
      forPercentage: 19.0, isScreenActive: false
    ).usingColorSpace(.sRGB)!
    assertTrue(
      inactiveRed.redComponent >= 0.85,
      "Inactive red menuBarColor for 19.0% must have redComponent >= 0.85")
    assertTrue(
      inactiveRed.redComponent <= activeRed.redComponent + 0.05,
      "Inactive red must not exceed active red by more than 0.05")
    ...
    assertTrue(
      sprintIcon.size.width >= 10.0 && sprintIcon.size.width <= 12.0,
      "Sprint icon width should fit 9 cols + shadow")
    assertTrue(
      sprintIcon.size.height >= 8.0 && sprintIcon.size.height <= 10.0,
      "Sprint icon height should fit 7 rows + shadow")
    assertTrue(
      weeklyIcon.size.width >= 8.0 && weeklyIcon.size.width <= 11.0,
      "Weekly icon width should fit 9 cols + shadow")
    assertTrue(
      weeklyIcon.size.height >= 8.0 && weeklyIcon.size.height <= 11.0,
      "Weekly icon height should fit 9 rows + shadow")
    ...
    assertTrue(
      weeklyDefault.tiffRepresentation != nil, "Default weekly icon must rasterize cleanly")
    ...
    // ====================================================================
    let greenColor = MenuBarAppearanceHelper.menuBarColor(
    ...
    assertTrue(
      activeSize.width > 20.0 && activeSize.width < 48.0,
      "Stacked values width must be compact (20-48pt), was \(activeSize.width)")
    ...
    assertTrue(
      tiffStacked != nil && tiffStacked!.count > 100, "Stacked image must rasterize to valid TIFF")
    ...
    assertTrue(
      stackedInactive.tiffRepresentation != nil, "Inactive stacked image must rasterize cleanly")
    ...
    assertEqual(
      attachment.bounds.origin.y, -6.0, "Attachment y origin must be -6.0 for baseline centering")
    assertEqual(attachment.bounds.size.height, 20.5, "Attachment height must be 20.5")
    assertEqual(
      attachment.bounds.size.width, activeSize.width, "Attachment width must match image width")
    ...
    assertTrue(
      textSize.width > activeSize.width,
      "Text fallback with '5h: '/'Wk: ' must be wider than icon mode")
    ...
    assertTrue(
      abs(topColor.redComponent - topColor.greenComponent) < 0.12,
      "Top sprint quota over exhausted weekly must be neutral slate gray, got R=\(topColor.redComponent), G=\(topColor.greenComponent)"
    )
    assertTrue(
      topColor.greenComponent < topColor.redComponent + 0.12,
      "Top sprint quota over exhausted weekly must NOT be green")
    ...
    assertTrue(
      leftEdgeColor.alphaComponent >= 0.95,
      "Rectangular badge: left edge (30% width) at row 1 must have alpha >= 0.95, got \(leftEdgeColor.alphaComponent)"
    )
    assertTrue(
      rightEdgeColor.alphaComponent >= 0.95,
      "Rectangular badge: right edge (70% width) at row 1 must have alpha >= 0.95, got \(rightEdgeColor.alphaComponent)"
    )
    ...
    let stratTopColor = stratBmp.colorAt(x: Int(Double(stratBmp.pixelsWide) * 0.50), y: stratTopY)!
      .usingColorSpace(.sRGB)!
    assertTrue(
      stratTopColor.greenComponent > stratTopColor.redComponent + 0.1,
      "Stratified top row (5h=80%) must be green, got R=\(stratTopColor.redComponent), G=\(stratTopColor.greenComponent)"
    )

    // Middle row (weekly @ 35% = yellow): sample at ~55% from top → yellow/amber dominant
    let stratMidY = Int(Double(stratBmp.pixelsHigh) * 0.55)
    let stratMidColor = stratBmp.colorAt(x: Int(Double(stratBmp.pixelsWide) * 0.50), y: stratMidY)!
      .usingColorSpace(.sRGB)!
    assertTrue(
      stratMidColor.redComponent > 0.5,
      "Stratified mid row (weekly=35%) must have strong red component for yellow/amber, got R=\(stratMidColor.redComponent)"
    )
    assertTrue(
      stratMidColor.greenComponent > 0.2,
      "Stratified mid row (weekly=35%) must have green component for yellow/amber, got G=\(stratMidColor.greenComponent)"
    )
    ...
    let stratBotColor = stratBmp.colorAt(x: Int(Double(stratBmp.pixelsWide) * 0.50), y: stratBotY)!
      .usingColorSpace(.sRGB)!
    assertTrue(
      stratBotColor.redComponent > 0.3,
      "Stratified bottom strip (credits=0) must have warm tone, got R=\(stratBotColor.redComponent)"
    )

    print(
      "  ✅ 3-Row Hybrid Shield Badge with exhausted weekly, rectangular shape, and stratified rows verified"
    )
    ...
    let activeTraits =
      activeNumFont.fontDescriptor.object(forKey: .traits) as? [NSFontDescriptor.TraitKey: Any]
    let inactiveTraits =
      inactiveNumFont.fontDescriptor.object(forKey: .traits) as? [NSFontDescriptor.TraitKey: Any]
    let activeWeight = activeTraits?[.weight] as? CGFloat ?? 0.0
    let inactiveWeight = inactiveTraits?[.weight] as? CGFloat ?? 0.0
    assertTrue(
      inactiveWeight <= activeWeight, "Inactive font weight must not exceed active font weight")
    ...
    assertTrue(
      inactiveShadowCol.alphaComponent <= activeShadowCol.alphaComponent,
      "Inactive shadow must be softer than active to eliminate lantern glow")
    assertEqual(inactiveShadow.shadowBlurRadius, 1.0)

    let activeSepColor = MenuBarAppearanceHelper.separatorColor(isScreenActive: true)
      .usingColorSpace(.sRGB)!
    let inactiveSepColor = MenuBarAppearanceHelper.separatorColor(isScreenActive: false)
      .usingColorSpace(.sRGB)!
    assertTrue(
      inactiveSepColor.redComponent <= activeSepColor.redComponent,
      "Inactive separator must not be brighter than active")
    ...
    let bracketTraits =
      bracketFontActive.fontDescriptor.object(forKey: .traits) as? [NSFontDescriptor.TraitKey: Any]
    let bracketWidth =
      bracketTraits?[NSFontDescriptor.TraitKey(rawValue: "NSCTFontWidthTrait")] as? CGFloat
    ...
    assertEqual(
      bracketShadowInactive.shadowBlurRadius, 1.0, "Inactive bracket shadow blur must be 1.0")

    let bracketShadowInactiveCol = (bracketShadowInactive.shadowColor ?? .black).usingColorSpace(
      .sRGB)!
    assertTrue(
      bracketShadowInactiveCol.alphaComponent >= 0.85,
      "Inactive bracket shadow alpha must be >= 0.85, got \(bracketShadowInactiveCol.alphaComponent)"
    )
    ...
    let redMenuBarCol = MenuBarAppearanceHelper.menuBarColor(
      forPercentage: 10.0, isScreenActive: true)
    let greenMenuBarCol = MenuBarAppearanceHelper.menuBarColor(
      forPercentage: 100.0, isScreenActive: true)
    let yellowMenuBarCol = MenuBarAppearanceHelper.menuBarColor(
      forPercentage: 35.0, isScreenActive: true)
    assertTrue(
      MenuBarAppearanceHelper.isRedColor(redMenuBarCol), "10% quota color must be classified as red"
    )
    assertTrue(
      !MenuBarAppearanceHelper.isRedColor(greenMenuBarCol),
      "100% quota color must NOT be classified as red")
    assertTrue(
      !MenuBarAppearanceHelper.isRedColor(yellowMenuBarCol),
      "35% quota color must NOT be classified as red")
    ...
    assertEqual(
      redShadowActive.shadowOffset.width, 0.0,
      "Red text shadow offset X must be 0 (omnidirectional)")
    assertEqual(
      redShadowActive.shadowOffset.height, 0.0,
      "Red text shadow offset Y must be 0 (omnidirectional)")
    assertTrue(
      redShadowActive.shadowBlurRadius > 0.0, "Red text shadow blur must be > 0 for soft spread")
    ...
    assertTrue(
      redShadowInactiveCol.alphaComponent > 0.0, "Inactive red text shadow alpha must be > 0")
    assertTrue(
      redShadowInactiveCol.alphaComponent <= redShadowActiveCol.alphaComponent,
      "Inactive red shadow must not be stronger than active")
    ...
    let redFiveHColor = MenuBarAppearanceHelper.menuBarColor(
      forPercentage: 5.0, isScreenActive: true)
    ...
    assertTrue(
      redStackedSize.width > 0 && redStackedSize.height > 0,
      "Red stacked image must have positive dimensions")
