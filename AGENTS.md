...
- Tests must never read, lock, or write live Desktop state (`~/.codex`, ChatGPT.app, IPC) or reach the network. A test that resolves `codex_home()` holds one `storage::test_codex_home::TestCodexHome` (never nested, and never alongside a raw `TEST_CODEX_HOME_MUTEX` lock; `TestEnv` already owns one). The guard serializes on `TEST_CODEX_HOME_MUTEX`, points `CODEX_HOME` at a fresh temporary directory, and clears it while unwinding. In test builds `codex_home()` panics unless `CODEX_HOME` is exactly the currently held guard's home; worker threads the test spawns may resolve it, and `tests/enforce_codex_home_isolation.rs` rejects any other `set_var`/`remove_var` of `CODEX_HOME`. Inject network-backed work such as quota-cache or status refreshes rather than letting a test reach it; `catch_unwind` or a discarded `Result` hides a real request. Probing a writer lock with `try_lock_exclusive` briefly takes it.
...
| Desktop owner discovery | up to `90 s` | Resolves the Desktop window that owns a task; a task URL is sent only when no owner exists. |
...
- **Owner discovery**: `cxi` connects to `~/.codex/ipc/ipc.sock`, asks `thread-owner-discovery` for the task owner, and activates ChatGPT once with a `codex://threads/<tid>` deep link only when Desktop reports no owner for a cold task. Ordinary `open -g -a /Applications/ChatGPT.app` URL retries request background delivery, but the current Desktop build can still bring ChatGPT to the foreground. Already-owned tasks are never cycled through the UI. URL acceptance is never a substitute for owner proof.
...
- **Cold-task navigation limit**: A successful macOS `open` exit for a deep link does not prove the task mounted. Historical logs show URL-to-owner-to-IPC recovery, but current live checks also found accepted links that stayed ownerless. In installed ChatGPT build 26.924.20706, ordinary task deep-link handling ensures the primary window is visible before navigation. Both `open -g -a` and a native LaunchServices experiment brought ChatGPT to the foreground despite background flags; the native experiment was rejected because it also failed to mount one cold task. No supported background cold-task mount IPC method was evident. Keep automatic switching disabled pending focus-safe, end-to-end, and multiwindow proof. Report partial recovery and never dispatch to an unverified owner; inspect the turn before a later explicit `cxi resume <id>`.
