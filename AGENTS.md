...
- **APP/CLI Identity Invariant**: Resolve APP quota only from a session marker bound to the exact live ChatGPT PID and birth identity, with a timestamp within that process lifetime. A running but unverified APP displays `—`; never borrow the CLI quota or a previous process marker. Under the operation lock, recheck the account registry and active CLI authentication against the decision inputs. Write the new APP binding before recovery waits, keep its CLI binding aligned with the staged auth during recovery, update that binding after CLI commit, and refresh the UI on marker replacement or Desktop process lifecycle events. On a required write failure after stopping Desktop, attempt to restore the previous session before returning. Process identity alone cannot detect an account change inside one unchanged Desktop process; do not claim that scenario is verified without an authoritative Desktop account read.
