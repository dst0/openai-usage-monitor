...
- **Window task diagnostic**: `cxi window probe-tasks --allow-focus-and-clipboard` explicitly foregrounds every standard ChatGPT window and uses Copy deeplink, leaving the final link on the clipboard. It checks unique AX-to-WindowServer geometry, stable exact process/window mapping, one observed clipboard write per copy, and unique canonical task IDs. The helper returns no task IDs and CLI prints only a count. A competing single valid-link clipboard write cannot be distinguished from the shortcut; this probe saves no snapshot and never authorizes restart or auto-switch. Targeted post-relaunch navigation and owner-client-to-window identity remain unproven.
