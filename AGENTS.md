...
- **Window task diagnostic**: `cxi window probe-tasks --allow-focus-and-clipboard` is explicit evidence gathering, never automation. It foregrounds every standard ChatGPT window and sends Copy deeplink, leaving the final link on the clipboard. Before any visible change it holds the switch/recovery operation lock for the whole run and requires a default ChatGPT keymap (`$CODEX_HOME/keybindings.json` absent, blank, or `[]`), one ChatGPT process, Accessibility and event-posting access, and no minimized window, because an override can rebind Cmd+Opt+L and Desktop copies from a fallback window when none is focused. Post the shortcut only to the verified PID, never at the HID tap; read focus live from Accessibility, not `NSWorkspace`, before and after each copy; require unique AX-to-WindowServer geometry, stable exact process/window mapping, one clipboard write that is unchanged across the read, and unique canonical task IDs. The helper returns no task IDs; the CLI prints only a count or a fixed failure code. A competing single valid-link clipboard write cannot be distinguished from the shortcut. Only `WindowTaskProbeService` may call the helper command (`tests/enforce_task_probe_isolation.rs`); this probe saves no snapshot and never authorizes restart or auto-switch. Targeted post-relaunch navigation and owner-client-to-window identity remain unproven.
