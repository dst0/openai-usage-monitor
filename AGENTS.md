...
- **Window task diagnostic**: `cxi window probe-tasks --allow-focus-and-clipboard` is explicit evidence gathering, never automation. It foregrounds every standard ChatGPT window and sends Copy deeplink; ChatGPT leaves each link on the system clipboard. Before any visible change it requires the account's `~/.codex` as the Codex home (from the user database, not `$HOME`), holds the switch/recovery operation lock for the whole run, and requires a default keymap (`keybindings.json` absent, blank, or `[]`), one ChatGPT process, the inspected build (26.924.20706, 11431) unchanged since launch, no App Shortcut on Cmd+Opt+L, a layout where that key types `l` with Command, Accessibility and event-posting access, pasteboard access not denied, and no minimized window, because an override, another build, or another layout can give Cmd+Opt+L a different meaning, and Desktop copies from a fallback window when none is focused. A keymap edit still present at the end voids the result. Post the shortcut only to the verified PID, never at the HID tap; read focus live from Accessibility, not `NSWorkspace`, before and after each copy; require unique AX-to-WindowServer geometry, stable exact process/window mapping, one clipboard write that is unchanged across the read and carries no concealed or transient marker, and unique canonical task IDs. Keep the helper's sequencing in `WindowTaskProbe` behind `WindowTaskProbeSystem` so a scripted fake tests it. The helper never outputs task IDs; the CLI prints only a count or a fixed failure code, and says when a failure may have followed a focus change. A competing single valid-link clipboard write cannot be distinguished from the shortcut. Only `WindowTaskProbeService` may call the helper command (`tests/enforce_task_probe_isolation.rs`); this probe saves no snapshot and never authorizes restart or auto-switch. Targeted post-relaunch navigation and owner-client-to-window identity remain unproven.
...
- **IPC reply reading**: XNU rejects `setsockopt` with `EINVAL` once the router has closed its end, even while its complete reply is still buffered. When re-arming the response read timeout, tolerate only that kernel `EINVAL` and read on; such a socket returns the buffered bytes and then EOF without blocking. Any other failure, including Rust's refusal of a zero timeout, is an error because it would leave the read unbounded.
