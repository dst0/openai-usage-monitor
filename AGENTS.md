...
- Tests must never read, lock, or write live Desktop state (`~/.codex`, ChatGPT.app, IPC) or reach the network. A test that resolves `codex_home()` holds one `storage::test_codex_home::TestCodexHome` on the calling thread. The guard serializes on `TEST_CODEX_HOME_MUTEX`, points `CODEX_HOME` at a fresh temporary directory, and clears it while unwinding. In test builds `codex_home()` panics unless `CODEX_HOME` is exactly the home of the guard that the calling thread holds, and `tests/enforce_codex_home_isolation.rs` rejects any other `set_var`/`remove_var` of `CODEX_HOME`. Inject network-backed work such as quota-cache or status refreshes rather than letting a test reach it; `catch_unwind` or a discarded `Result` hides a real request. Probing a writer lock with `try_lock_exclusive` briefly takes it.
